...
import com.piggymetrics.statistics.domain.Constant;
...
public class ExchangeRatesServiceImplTest {
	...
	public void shouldReturnCurrentRatesWhenContainerIsEmptySoFar() {
		...
		when(client.getRates( Constant.access_key, Constant.symbols,Currency.getBase())).thenReturn(container);

		Map<Currency, BigDecimal> result = ratesService.getCurrentRates();
		verify(client, times(1)).getRates( Constant.access_key, Constant.symbols,Currency.getBase());
